...
- [x] FORM-015 browser-review follow-up moved the controlled form code from the header into a subtle bottom-right document footer. Signature display now uses only a valid selected Team Leader/Sub Leader and falls back to the authoritative Team Leader when no signer is saved; Draft workspaces also preselect that Team Leader as an unsaved UX default. No database or migration change was required.
