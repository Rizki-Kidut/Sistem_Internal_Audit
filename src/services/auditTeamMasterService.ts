...
function mapTeam(row: Record<string, unknown>): AuditTeamMaster {
  const members=((row.members ?? []) as Record<string, unknown>[]).map((member) => ({
    ...
    is_lead_auditor: Boolean(member.is_lead_auditor ?? member.peran === AUDIT_TEAM_MEMBER_ROLE.LEAD),
...
export async function saveAuditTeamMaster(team: Partial<AuditTeamMaster>): Promise<AuditTeamMaster> {
  ...
  if (team.status === AUDIT_TEAM_MASTER_STATUS.AKTIF) {
    if (members.filter((m) => m.is_team_leader).length !== 1) throw new Error('Tim Audit aktif harus memiliki tepat satu Team Leader');
    if (members.filter((m) => m.is_lead_auditor).length !== 1) throw new Error('Tim Audit aktif harus memiliki tepat satu Lead Auditor');
  }
  const { data, error } = await supabase.rpc('save_audit_team_master', { p_id: team.id || null, p_plan_id:team.plan_id,p_kode_tim: team.kode_tim,
    ...
    p_members: members.map((m) => ({ auditor_id: m.auditor_id, peran: m.is_team_leader ? AUDIT_TEAM_MEMBER_ROLE.LEAD : AUDIT_TEAM_MEMBER_ROLE.MEMBER,
      is_team_leader: m.is_team_leader, is_lead_auditor: m.is_lead_auditor, urutan_tampil: m.urutan_tampil })) });
...
export async function assignTeamToInstructionRow(rowId: string, teamMasterId: string | null, justification: string): Promise<void> {
  ...
  if (team.members.filter((m) => m.is_lead_auditor).length !== 1) throw new Error('Tim Audit harus memiliki tepat satu Lead Auditor');
