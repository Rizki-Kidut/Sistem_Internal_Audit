...
const teamLeaderNames=(members:AuditTeamMasterMember[])=>members.filter(m=>m.is_team_leader).map(m=>m.auditor?.nama??m.auditor_id).join(', ')||'-';
const regularNames=(members:AuditTeamMasterMember[])=>members.filter(m=>!m.is_team_leader).map(m=>m.auditor?.nama??m.auditor_id).join(', ')||'-';
...
function TeamForm({team,auditors,onChange}:{team:AuditTeamMaster;auditors:Auditor[];onChange:(t:AuditTeamMaster)=>void}){
 ...
 const members=team.members.filter(m=>!m.is_team_leader).map(m=>m.auditor_id);
 ...
 const rebuild=(nextTeamLeader:string,nextMembers:string[])=>{
   const ids=Array.from(new Set([nextTeamLeader,...nextMembers].filter(Boolean)));
   ...
   const rebuilt=ids.map((id,index)=>{
     ...
     return {id:existing?.id??'',team_id:team.id,auditor_id:id,peran:isTeamLeader?AUDIT_TEAM_MEMBER_ROLE.LEAD:AUDIT_TEAM_MEMBER_ROLE.MEMBER,is_team_leader:isTeamLeader,urutan_tampil:index,auditor:existing?.auditor};
   });
   onChange({...team,members:rebuilt});
 };
 const setTeamLeader=(id:string)=>rebuild(id,members.filter(memberId=>memberId!==id));
 const setMembers=(ids:string[])=>rebuild(teamLeader,ids.filter(id=>id!==teamLeader));
 return <div className="space-y-3"><div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs text-blue-800">Setiap Tim Audit memiliki tepat satu Team Leader untuk koordinasi dan Submit/Resubmit PLOR. Lead Auditor adalah authority perusahaan dan tidak dipilih di master Tim Audit.</div><div className="grid grid-cols-2 gap-3"><Field label="Kode Tim" required><Input disabled={disabled} value={team.kode_tim} onChange={e=>onChange({...team,kode_tim:e.target.value})}/></Field><Field label="Nama Tim" required><Input disabled={disabled} value={team.nama_tim} onChange={e=>onChange({...team,nama_tim:e.target.value})}/></Field><Field label="Team Leader" required><SearchableSelect disabled={disabled} options={options} value={teamLeader} onChange={setTeamLeader} placeholder="Cari Team Leader..."/></Field><Field label="Status"><Select disabled={disabled} value={team.status} onChange={e=>onChange({...team,status:e.target.value as AuditTeamMaster['status']})}><option>Aktif</option><option>Nonaktif</option></Select></Field></div><Field label="Member"><SearchableMultiSelect disabled={disabled} options={options.filter(o=>o.value!==teamLeader)} values={members} onChange={setMembers}/></Field><Field label="Catatan"><Textarea disabled={disabled} value={team.catatan??''} onChange={e=>onChange({...team,catatan:e.target.value||null})}/></Field></div>;
}
