...
export function LtpManagerReview({context,onRefresh}:{context:LtpWorkflowContext;onRefresh:()=>Promise<void>}){
  ...
  const decision=[...context.workflow_events].reverse().find(event=>event.event_type==='MANAGER_RETURNED_TO_AUDITEE'||event.event_type==='MANAGER_APPROVED_TO_AUDITOR');
  if(!isManagerReview&&!decision)return null;
  const isManagerReturned=decision?.event_type==='MANAGER_RETURNED_TO_AUDITEE';
