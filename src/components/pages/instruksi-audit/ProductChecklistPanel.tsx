...
import { CHECKLIST_PRODUK_STATUS } from '../../../lib/enums';
import {
  ...
  saveProductChecklist, saveProductItemPreparation, saveProductPhase, uploadProductEvidence,
...
export function ProductChecklistPanel({ row, readOnly, onError, onChanged }: Props) {
  ...
  async function saveItem() {
    ...
    try { await saveProductItemPreparation(itemForm); setItemSaveError(null); onError(''); setItemForm(null); await loadDetails(active); await onChanged?.(); }
...
function PhaseCard({ phase, items, editable, onEdit, onDelete, onAddItem, onEditItem, onDeleteItem, onUpload, onOpen, onDeleteEvidence }: {
  ...
  return <div className="border border-gray-200 rounded-lg overflow-hidden"><div className="p-3 bg-gray-50 flex justify-between gap-3"><div><div className="font-medium text-gray-900">{phase.nama_fase}</div><div className="text-xs text-gray-500">{phase.nama_proses || '-'} · Chart: {phase.inspection_result_chart ? 'Ya' : 'Tidak'} · Standard: {phase.no_inspection_standard || '-'}</div></div>{editable && <div className="flex"><Button size="sm" variant="ghost" onClick={onEdit}><Pencil size={13} /></Button><Button size="sm" variant="ghost" onClick={onDelete}><Trash2 size={13} /></Button></div>}</div>
    ...
    <div className="p-3 overflow-x-auto"><div className="flex justify-between mb-2"><div><span className="text-sm font-medium">Item Pemeriksaan ({items.length})</span><p className="text-xs text-gray-500">Persiapan Inspection Checklist; hasil aktual diisi melalui Pelaksanaan Audit.</p></div>{editable && <Button size="sm" onClick={onAddItem}><Plus size={13} /> Tambah Item</Button>}</div><table className="min-w-[850px] w-full text-xs"><thead><tr className="bg-gray-50 text-left">{['Kategori','Sampel Minimal','Item Pemeriksaan','Alat','Standar/Kriteria','Urutan','Aksi'].map((h) => <th key={h} className="p-2">{h}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t"><td className="p-2">{item.kategori || '-'}</td><td className="p-2">{item.jumlah_sampel_minimal ?? '-'}</td><td className="p-2">{item.item_pemeriksaan}</td><td className="p-2">{item.alat_pemeriksaan || '-'}</td><td className="p-2">{item.standar_kriteria || '-'}</td><td className="p-2">{item.urutan_tampil}</td><td className="p-2">{editable && <><button className="text-blue-600 mr-2" onClick={() => onEditItem(item)}><Pencil size={13} /></button><button className="text-red-500" onClick={() => onDeleteItem(item)}><Trash2 size={13} /></button></>}</td></tr>)}</tbody></table></div>
  </div>;
}
