...
import {
  CHECKLIST_BANK_STATUS, CHECKLIST_MANUFAKTUR_STATUS,
} from '../../../lib/enums';
import {
  ...
  saveManufacturingBankItem, saveManufacturingChecklist, saveManufacturingItemPreparation,
...
export function ManufacturingChecklistPanel({ row, auditorList, readOnly, onError, onChanged }: Props) {
  ...
  return <div className="space-y-4">
    ...
    <Card className="overflow-x-auto"><div className="p-4 flex justify-between"><div><h3 className="font-semibold">Item Checklist ({items.length})</h3><p className="text-xs text-gray-500">Persiapan FORM-007; hasil Genba diisi melalui Pelaksanaan Audit.</p></div>{editable && <div className="flex gap-2"><Button size="sm" variant="secondary" onClick={syncBank}>Sinkronkan Bank Aktif</Button><Button size="sm" onClick={() => {setItemSaveError(null);setItemForm(emptyManualItem(active.id));}}><Plus size={14} /> Item Manual</Button></div>}</div><table className="min-w-[750px] w-full text-xs"><thead><tr className="bg-gray-50 text-left">{['No.','Klausul','Item Pemeriksaan','No. Proses Dicek','Urutan','Aksi'].map((h) => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t"><td className="p-3">{item.bank_item ? `${item.bank_item.bagian}-${item.bank_item.nomor}` : 'Manual'}</td><td className="p-3">{item.bank_item?.klausul || '-'}</td><td className="p-3">{item.bank_item?.item_pemeriksaan || <span className="text-amber-600">Belum dilengkapi di bank</span>}</td><td className="p-3">{item.no_proses_dicek || '-'}</td><td className="p-3">{item.urutan_tampil}</td><td className="p-3">{editable && <><button className="text-blue-600 mr-3" onClick={() => {setItemSaveError(null);setItemForm(item);}}><Pencil size={14} /></button><button className="text-red-500" onClick={() => setDeleteTarget({ type: 'item', id: item.id, label: 'item ini' })}><Trash2 size={14} /></button></>}</td></tr>)}</tbody></table></Card>
    ...
    <Modal open={!!itemForm} onClose={() => {setItemForm(null);setItemSaveError(null);}} title="Edit Struktur Item Checklist" footer={<><Button variant="secondary" onClick={() => {setItemForm(null);setItemSaveError(null);}}>Batal</Button><Button onClick={saveItem}>Simpan</Button></>}>{itemSaveError&&<div className="mb-3 p-3 bg-red-50 border border-red-200 text-red-700"><p className="font-semibold text-sm">Gagal menyimpan Checklist</p><p className="text-sm mt-1">{itemSaveError}</p></div>}{itemForm && <div className="space-y-3"><Field label="No. Proses Dicek"><Input value={itemForm.no_proses_dicek ?? ''} onChange={(e) => setItemForm({ ...itemForm, no_proses_dicek: e.target.value || null })} /></Field><Field label="Urutan Tampil"><Input type="number" min="0" value={itemForm.urutan_tampil} onChange={(e) => setItemForm({ ...itemForm, urutan_tampil: Number(e.target.value) })} /></Field></div>}</Modal>
    <Modal open={bankOpen} onClose={() => setBankOpen(false)} title="Bank Checklist Manufaktur" size="xl"><div className="flex justify-end mb-3">{editable && <Button size="sm" onClick={() => setBankForm(emptyBank())}><Plus size={14} /> Tambah</Button>}</div><div className="max-h-96 overflow-auto"><table className="w-full text-xs"><thead><tr className="bg-gray-50 text-left"><th className="p-2">Nomor</th><th>Klausul</th><th>Item Pemeriksaan</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{bank.map((item) => <tr key={item.id} className="border-t"><td className="p-2">{item.bagian}-{item.nomor}</td><td>{item.klausul || '-'}</td><td>{item.item_pemeriksaan || 'Belum dilengkapi'}</td><td>{item.status}</td><td>{editable && <><button className="text-blue-600 mr-2" onClick={() => setBankForm(item)}><Pencil size={13} /></button>{item.status === 'Aktif' && <button className="text-red-500" onClick={() => setDeleteTarget({ type: 'bank', id: item.id, label: `${item.bagian}-${item.nomor}` })}><Trash2 size={13} /></button>}</>}</td></tr>)}</tbody></table></div></Modal>
