...
export function ManufacturingChecklistPanel({ row, auditorList, readOnly, onError }: Props) {
  ...
  const auditorNames = row.auditor.map((assignment) => auditorList.find((auditor) => auditor.id === assignment.auditor_id)?.nama ?? assignment.auditor_id).join(', ');
  ...
  return <div className="space-y-4">
    <Card className="p-4"><div className="flex justify-between gap-3 flex-wrap"><div><div className="flex gap-2 items-center"><Badge variant="blue">{active.kode_audit}</Badge><Badge variant={active.status === 'Selesai' ? 'green' : 'gray'}>{active.status}</Badge></div><p className="text-xs text-gray-500 mt-1">{active.kode_dokumen}</p></div><div className="flex gap-2">
      ...
      <div className="grid md:grid-cols-3 gap-3 mt-4 text-sm">{[
        ['Jenis Checklist', active.jenis_checklist.map((j) => `${j.plant_nama} / ${j.shift_nama}`).join(', ') || '-'], ['Nama Seksi', active.nama_seksi || '-'], ['Manager Proses / Line Leader', active.manager_proses_line_leader || '-'], ['Tanggal Audit', active.tanggal_audit || '-'], ['Auditor', auditorNames || '-'], ['Nama Part', active.nama_part || '-'], ['Nomor Part', active.nomor_part || '-'], ['Nomor Line', active.nomor_line || '-'], ['Control Plan No.', active.control_plan_no || '-'], ['P-FMEA No.', active.p_fmea_no || '-'], ['Customer', active.customer || '-'], ['Jumlah Operator', active.jumlah_operator ?? '-'],
    ...
    <Modal open={editHeader} onClose={() => setEditHeader(false)} title="Edit Header Checklist Manufaktur/Shift" size="xl" footer={<><Button variant="secondary" onClick={() => setEditHeader(false)}>Batal</Button><Button onClick={() => saveHeader()}>Simpan</Button></>}><HeaderForm draft={draft} suggestions={suggestions} onChange={setDraft} auditorNames={auditorNames} /></Modal>
