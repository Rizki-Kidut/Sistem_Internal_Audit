...
export function ProductAuditExecutionPanel({ row, readOnly, onError, onChanged }: Props) {
  ...
  return <div className="space-y-4">{!preparationReady && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Checklist Produk belum Siap Pelaksanaan. Selesaikan persiapan Checklist Audit terlebih dahulu.</div>}<div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800">Pelaksanaan Produk hanya mencatat sampel aktual, hasil pemeriksaan, dan Judgment. Struktur inspeksi dikelola di Checklist Audit.</div>{phases.map(phase => <Card key={phase.id} className="overflow-hidden"><div className="p-4 bg-gray-50 border-b"><h3 className="font-semibold">{phase.name}</h3><p className="text-xs text-gray-500">{phase.process || '-'} · Inspection Standard: {phase.standard || '-'}</p></div><div className="divide-y">{phase.items.map(item => { const draft=drafts[item.id]; const evaluated=Boolean(draft?.judgment&&draft.hasil_pemeriksaan.trim()); return <div key={item.id} className="p-4"><div className="grid lg:grid-cols-[minmax(220px,1fr)_130px_minmax(220px,0.8fr)_170px_170px_90px] gap-3 items-start"><div><p className="text-xs text-gray-500">{item.kategori || 'Tanpa kategori'} · Sampel minimal: {item.jumlah_sampel_minimal ?? '-'}</p><p className="font-medium mt-1">{item.item_pemeriksaan}</p><p className="text-xs text-gray-500 mt-1">Alat: {item.alat_pemeriksaan || '-'} · Standar/Kriteria: {item.standar_kriteria || '-'}</p></div><Field label="Sampel Aktual"><Input disabled={executionReadOnly} type="number" min="0" value={draft?.jumlah_sampel ?? ''} onChange={event=>setDrafts({...drafts,[item.id]:{...draft,jumlah_sampel:event.target.value===''?null:Number(event.target.value)}})}/></Field><Field label="Hasil Pemeriksaan *"><Textarea disabled={executionReadOnly} rows={3} value={draft?.hasil_pemeriksaan ?? ''} onChange={event=>setDrafts({...drafts,[item.id]:{...draft,hasil_pemeriksaan:event.target.value}})}/></Field><Field label="Judgment *"><Select disabled={executionReadOnly} value={draft?.judgment ?? ''} onChange={event=>{const judgment=(event.target.value||null) as JudgmentProduk|null;setDrafts({...drafts,[item.id]:{...draft,judgment,finding_kategori:judgment===JUDGMENT_PRODUK.NG?draft.finding_kategori:null}})}}><option value="">-- Pilih Judgment --</option>{JUDGMENT_PRODUK_LIST.map(value=><option key={value}>{value}</option>)}</Select></Field><Field label="Kategori Temuan"><Select disabled={executionReadOnly||draft?.judgment!==JUDGMENT_PRODUK.NG} value={draft?.finding_kategori??''} onChange={event=>setDrafts({...drafts,[item.id]:{...draft,finding_kategori:(event.target.value||null) as KategoriTemuan|null}})}><option value="">-- Pilih --</option><option value="A">A — Major</option><option value="B">B — Minor</option><option value="C">C — OFI</option></Select></Field><div className="flex lg:flex-col gap-2 lg:mt-6"><Badge variant={evaluated?'green':'amber'}>{evaluated?'Dinilai':'Belum'}</Badge>{!executionReadOnly&&<Button size="sm" disabled={savingId===item.id} onClick={()=>save(item)}>{savingId===item.id?'Menyimpan...':'Simpan'}</Button>}</div></div></div>})}</div></Card>)}</div>;
}
