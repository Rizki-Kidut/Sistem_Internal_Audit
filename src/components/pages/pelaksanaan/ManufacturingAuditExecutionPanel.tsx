...
export function ManufacturingAuditExecutionPanel({ row, readOnly, onError, onChanged }: Props) {
  ...
  return <div className="space-y-3"><div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800">Pelaksanaan Genba hanya mencatat Hasil Pengamatan dan Judgement per item. Bank, header, dan struktur FORM-007 dikelola di Checklist Audit.</div>{items.map((item,index)=>{const draft=drafts[item.id];const evaluated=Boolean(draft?.hasil&&draft.hasil_pengamatan.trim());return <Card key={item.id} className="p-4"><div className="grid lg:grid-cols-[minmax(240px,1fr)_minmax(240px,0.9fr)_190px_90px] gap-4"><div><p className="text-xs text-gray-500">{index+1} / {items.length} · {item.bank_item?`${item.bank_item.bagian}-${item.bank_item.nomor}`:'Manual'} · Klausul: {item.bank_item?.klausul||'-'}</p><p className="font-medium mt-1">{item.bank_item?.item_pemeriksaan||'Item manual'}</p><p className="text-xs text-gray-500 mt-1">No. Proses Dicek: {item.no_proses_dicek||'-'}</p></div><Field label="Hasil Pengamatan *"><Textarea disabled={readOnly} rows={4} value={draft?.hasil_pengamatan??''} onChange={event=>setDrafts({...drafts,[item.id]:{...draft,hasil_pengamatan:event.target.value}})} placeholder="Catat bukti atau alasan N-A untuk item ini"/></Field><Field label="Judgement *"><Select disabled={readOnly} value={draft?.hasil??''} onChange={event=>setDrafts({...drafts,[item.id]:{...draft,hasil:(event.target.value||null) as HasilChecklist|null}})}><option value="">-- Pilih Judgement --</option>{HASIL_CHECKLIST_LIST.map(result=><option key={result} value={result}>{HASIL_CHECKLIST_LABEL[result]}</option>)}</Select></Field><div className="flex lg:flex-col gap-2 lg:mt-6"><Badge variant={evaluated?'green':'amber'}>{evaluated?'Dinilai':'Belum'}</Badge>{!readOnly&&<Button size="sm" disabled={savingId===item.id} onClick={()=>save(item)}>{savingId===item.id?'Menyimpan...':'Simpan'}</Button>}</div></div></Card>})}</div>;
}
