...
export function LtpPage(){
  ...
  return <div><h1 className="text-2xl font-bold">LTP — Laporan Tindakan Perbaikan</h1><p className="text-sm text-gray-500 mb-6">Satu Finding memiliki maksimal satu LTP. No. LTP mengikuti No. Temuan.</p>
    {error&&<div className="p-3 mb-4 bg-red-50 text-red-700 rounded-lg">{error}</div>}
    <Card className="p-4 mb-4"><div className="grid md:grid-cols-2 gap-3"><Input placeholder="Cari No. LTP, audit, seksi, atau proses" value={search} onChange={e=>setSearch(e.target.value)}/><Select value={status} onChange={e=>setStatus(e.target.value)}><option value="">Semua Status</option>{Object.entries(LTP_STATUS_LABEL).map(([value,label])=><option key={value} value={value}>{label}</option>)}</Select></div></Card>
    {!filtered.length?<Card className="p-12"><EmptyState icon={<FileCheck/>} title="Belum ada LTP" message="LTP dibuat otomatis setelah Finding dipublikasikan dan PLOR lengkap."/></Card>:<Card className="overflow-x-auto"><table className="min-w-[1050px] w-full text-sm"><thead><tr className="bg-gray-50 text-left">{['No. LTP','No. Audit','Kategori','Seksi Auditee','Proses','Tanggal Temuan','Status','Aksi'].map(h=><th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{filtered.map(row=><tr key={row.car_id} className="border-t"><td className="p-3 font-mono font-semibold">{row.kode_ltp}</td><td>{row.kode_audit}</td><td><Badge variant={row.kategori==='A'?'red':row.kategori==='C'?'blue':'green'}>{row.kategori}</Badge></td><td>{row.seksi_nama??'-'}</td><td>{row.proses_nama??'-'}</td><td>{date(row.tanggal_temuan)}</td><td><Badge variant="gray">{LTP_STATUS_LABEL[row.status]}</Badge></td><td><Button size="sm" onClick={()=>setSelected(row.car_id)}>Buka LTP</Button></td></tr>)}</tbody></table></Card>}
  </div>;
}
