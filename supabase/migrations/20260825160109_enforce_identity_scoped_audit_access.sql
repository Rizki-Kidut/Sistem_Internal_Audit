...
CREATE POLICY auditors_scoped_read ON public.auditors FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR id=public.current_auditor_id() OR public.current_auditor_is_peer(id) OR public.manager_can_access_auditor(id));
...
CREATE POLICY instruction_rows_auditor_read ON public.audit_instruction_rows FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.auditor_can_access_instruction_row(id));
...
CREATE POLICY team_masters_member_read ON public.audit_team_masters FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.current_auditor_belongs_to_team(id));
...
CREATE POLICY team_members_team_read ON public.audit_team_master_members FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.current_auditor_belongs_to_team(team_id));
...
CREATE POLICY checklists_auditor_read ON public.checklists FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.auditor_can_access_instruction_row(row_id));
...
CREATE POLICY checklist_items_auditor_read ON public.checklist_items FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR EXISTS(SELECT 1 FROM public.checklists c WHERE c.id=checklist_id AND public.auditor_can_access_instruction_row(c.row_id)));
...
CREATE POLICY product_auditor_read ON public.checklist_produk FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.auditor_can_access_instruction_row(row_id));
...
CREATE POLICY product_fase_auditor_read ON public.checklist_produk_fase FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR EXISTS(SELECT 1 FROM public.checklist_produk c WHERE c.id=checklist_produk_id AND public.auditor_can_access_instruction_row(c.row_id)));
...
CREATE POLICY product_items_auditor_read ON public.checklist_produk_items FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR EXISTS(SELECT 1 FROM public.checklist_produk_fase f JOIN public.checklist_produk c ON c.id=f.checklist_produk_id WHERE f.id=fase_id AND public.auditor_can_access_instruction_row(c.row_id)));
...
CREATE POLICY manufacturing_auditor_read ON public.checklist_manufaktur_shift FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.auditor_can_access_instruction_row(row_id));
...
CREATE POLICY manufacturing_items_auditor_read ON public.checklist_manufaktur_items FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR EXISTS(SELECT 1 FROM public.checklist_manufaktur_shift c WHERE c.id=checklist_id AND public.auditor_can_access_instruction_row(c.row_id)));
...
CREATE POLICY findings_auditor_read ON public.findings FOR SELECT TO authenticated USING(public.current_auditor_is_lead_auditor() OR public.auditor_can_access_instruction_row(instruction_row_id));
...
CREATE POLICY audit_evidence_scoped_select ON storage.objects FOR SELECT TO authenticated USING(bucket_id='audit-evidence' AND public.product_evidence_checklist_id(name) IS NOT NULL AND (public.is_admin_identity() OR public.current_auditor_is_lead_auditor() OR EXISTS(SELECT 1 FROM public.checklist_produk c WHERE c.id=public.product_evidence_checklist_id(name) AND public.auditor_can_access_instruction_row(c.row_id))));
