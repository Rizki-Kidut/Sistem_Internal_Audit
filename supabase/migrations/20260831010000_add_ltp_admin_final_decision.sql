-- Batch 7f: Admin/QMS final LTP decision and Auditor re-verification after Admin return.
...
BEGIN
  ...
  IF v_car.status NOT IN ('AUDITOR_REVIEW','AUDITOR_RETURNED') THEN
...
BEGIN
  ...
  IF NOT FOUND OR v_car.status NOT IN ('AUDITOR_REVIEW','AUDITOR_RETURNED') THEN
  ...
  WHERE finding_id=v_car.finding_id
    AND notification_type IN ('LTP_AUDITOR_REVIEW','LTP_AUDITOR_RETURNED')
    AND read_at IS NULL;
...
CREATE OR REPLACE FUNCTION public.ltp_admin_decision_blockers(p_car_id uuid,p_decision text)
RETURNS text[] LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE v_car public.cars%ROWTYPE; v_decision text:=upper(btrim(COALESCE(p_decision,''))); v_blockers text[]:=ARRAY[]::text[];
BEGIN
  SELECT * INTO v_car FROM public.cars WHERE id=p_car_id;
  IF NOT FOUND THEN RETURN ARRAY['LTP tidak ditemukan.']; END IF;
  IF v_car.status<>'ADMIN_REVIEW' THEN RETURN ARRAY['LTP tidak berada pada tahap keputusan Admin/QMS.']; END IF;
  IF v_decision='RETURN' THEN
    IF NOT EXISTS (SELECT 1 FROM public.user_profiles p WHERE public.auditor_user_can_receive_finding(p.id,v_car.finding_id)) THEN
      v_blockers:=array_append(v_blockers,'Belum ada Auditor aktif dengan penugasan tahunan yang dapat menerima LTP ini.');
    END IF;
  ELSIF v_decision='APPROVE' THEN
    IF v_car.auditor_verification_result IS DISTINCT FROM 'CLOSE' THEN v_blockers:=array_append(v_blockers,'LTP hanya dapat ditutup setelah hasil verifikasi Auditor Close.'); END IF;
  ELSE v_blockers:=array_append(v_blockers,'Keputusan Admin/QMS tidak valid.'); END IF;
  RETURN v_blockers;
END $$;
...
REVOKE ALL ON FUNCTION public.ltp_admin_decision_blockers(uuid,text) FROM PUBLIC,anon,authenticated;
